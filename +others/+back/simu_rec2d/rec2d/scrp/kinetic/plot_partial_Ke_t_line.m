...
outdir='/data/simulation/rec2d_M100SBg00Sx/out/line/kinetic/electron/partial_K_partial_t/';
tt=50:100;
di=40;
ndx=4800;
ndy=2400;
...
for t=1:nt
    ...
    linewidth=2;
    set(h,'Visible','off');
    [ax,h1,h2]=plotyy(lx,[lfx;lwe;lwp;ltot],lx,lbzh);
    % set y axes
    ...
    % set label
    set(get(ax(1),'Ylabel'),'String','dKe/dt','FontSize',fontsize);
    ...
    % set legend
    hl=legend([h1(:);h2],'Flux','Work by E field','Work by pressure','Total','Bz','Location','Best');
    set(gca,'FontSize',fontsize)
    %% save figures
    cd(outdir);
    print('-r200','-dpng',['partial_Ke_partial_t_Bz_t',num2str(th,'%06.2f'),'.png']);
    close(gcf);
end
