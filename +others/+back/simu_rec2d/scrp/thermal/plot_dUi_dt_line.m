...
outdir='/data/simulation/rec2d_M100SBg00Sx/out/thermal/ion/line/';
tt=0:0.5:100;
di=40;
ndx=4800;
ndy=2400;
...
for t=1:nt
    ...
    % set label
    set(get(ax(1),'Ylabel'),'String','dUi/dt','FontSize',fontsize);
    ...
    % set legend
    hl=legend([h1(:);h2],'-\nabla \cdot (u_{i}V_{i}+P_{i} \cdot V_{i})','-\nabla \cdot q_{i}','V_{i}\cdot (\nabla \cdot P_{i})','v_{RF}\cdot \nabla U_{i}','Sum','Bz','Location','SouthEast');
    %% save figures
    cd(outdir);
    print('-r300','-dpng',['dUi_dt_Bz_t',num2str(tm,'%06.2f'),'.png']);
    close(gcf);
end
