...
rate(t, 1) = sum(divPV.value(yrange, zrange), 'all');

%% V \cdot (\nabla \cdot P)
VdivP = V.dot(P.divergence(prm));
rate(t, 2) = sum(VdivP.value(yrange, zrange), 'all');
...
rate(t, 3) = sum(PdivV.value(yrange, zrange), 'all');
end
...
set(gca, 'FontSize', 14);
print('-dpng', '-r300', ['pressure_work_', sfx, '.png']);