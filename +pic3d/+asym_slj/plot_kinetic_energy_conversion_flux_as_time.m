...
for t=1:nt
    ...
    [top, bottom, left, right] = slj.Physics.integrate2d_flux(KV, xindex, zindex, prm);
    rate(1,t)=sum(divKV.value(zindex(1):zindex(2),xindex(1):xindex(2)),'all');
    rate(2,t)=top;
    rate(3,t)=bottom;
    rate(4,t)=left;
    rate(5,t)=right;
...
legend('-\nabla \cdot (KV)', 'top', 'bottom', 'left', 'right');
xlim(xrange);
xlabel('\Omega_{ci} t');
ylabel('-\int KV \cdot dl');
set(gca,'FontSize',14);

%% save
cd(outdir);
print('-dpng','-r300',[sfx,'_bulk_kinetic_conversion_as_time_dt=',num2str(dt),'_flux.png']);
