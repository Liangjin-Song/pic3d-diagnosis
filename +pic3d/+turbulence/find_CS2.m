...
%%选择坐标画向量
      ...
      V_x1=SPX(M,1)-15.*cosd(theta);
      V_x2=SPX(M,1)+15.*cosd(theta);
      V_y1=SPY(M,1)-15.*sind(theta);
      V_y2=SPY(M,1)+15.*sind(theta);
      
      V_x3=SPX(M,1)-15.*cosd(theta2);
      V_x4=SPX(M,1)+15.*cosd(theta2);
      V_y3=SPY(M,1)-15.*sind(theta2);
      V_y4=SPY(M,1)+15.*sind(theta2);
%%   
      TS_value=rms(J.y);
      TS_value=3*rms(TS_value);   %%%%%%%%%%
      Jy=J.y;
      Jy(find(abs(Jy)<TS_value))=0;
      [Jy_L,num_J]=bwlabel(Jy,8);  %%%%%%%%% 
      [Jy_Lx,Jy_Ly]=find(Jy_L~=0);
      linearindJy_L=sub2ind(size(J.y),Jy_Lx,Jy_Ly);
      MR=intersect(linearindJy_L,linearindSP);
